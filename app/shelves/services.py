...
class ShelfService:
    ...
    async def create_shelf(
        self, data: ShelfCreate, user_id: int, image: UploadFile | None
    ) -> Shelf:
    ...
    async def get_shelf_by_id(self, shelf_id: int, user_id: int | None) -> Shelf:
        result = await self.db.scalars(
            select(Shelf)
            .options(
                selectinload(Shelf.books).options(
    ...
    async def _get_count_shelves(self, filters: list) -> int:
        result = await self.db.scalar(select(func.count(Shelf.id)).where(*filters))
        return result or 0
    ...
    def _build_filters(self, current_user_id: int | None, **kwargs) -> list:
        ...
        if kwargs.get("user_id"):
            ...
            filters.append(
                or_(Shelf.is_private == False, Shelf.user_id == current_user_id)
            )
