...
async def test_create_author_by_admin_success(client, db_session, admin_headers):
    author_data = {"first_name": "Михаил", "last_name": "Булгаков", "bio": "Автор Мастера и Маргариты"}
    response = await client.post("/authors/", data=author_data, headers=admin_headers)

    assert response.status_code == 201

    result = await db_session.scalars(
        select(Author).where(Author.first_name == "Михаил", Author.last_name == "Булгаков")
    )
    assert result.first() is not None


@pytest.mark.asyncio
async def test_create_author_with_image_by_admin_success(client, db_session, admin_headers, mock_media_root):
    author_data = {
        "first_name": "Михаил",
        "last_name": "Булгаков",
    ...
    response = await client.post("/authors/", data=author_data, files=file_data, headers=admin_headers)
...
async def test_get_all_authors_excluded_inactive(client, db_session):
    ...
    inactive_author = Author(first_name="inactive", last_name="inactive", is_active=False)
...
@pytest.mark.asyncio
async def test_update_author_by_admin_success(client, db_session, existing_author, admin_headers):
    author_data = {"first_name": "new_first", "last_name": "new_last"}
    response = await client.patch(f"/authors/{existing_author.id}", json=author_data, headers=admin_headers)
...
async def test_update_author_by_admin_not_found(client, admin_headers):
    author_data = {"first_name": "new_first", "last_name": "new_last"}
    response = await client.patch("/authors/101", json=author_data, headers=admin_headers)
...
@pytest.mark.asyncio
async def test_update_author_by_regular_user_forbidden(client, existing_author, user_headers):
    author_data = {"first_name": "new_first", "last_name": "new_last"}
    response = await client.patch(f"/authors/{existing_author.id}", json=author_data, headers=user_headers)
    assert response.status_code == 403
...
@pytest.mark.asyncio
async def test_soft_delete_author_by_admin_success(client, db_session, existing_author, admin_headers):
    response = await client.delete(f"/authors/{existing_author.id}", headers=admin_headers)
...
@pytest.mark.asyncio
async def test_soft_delete_author_by_regular_user_forbidden(client, existing_author, user_headers):
    response = await client.delete(f"/authors/{existing_author.id}", headers=user_headers)
    assert response.status_code == 403
...
@pytest.mark.asyncio
async def test_update_author_image_by_admin_success(client, db_session, existing_author, admin_headers, mock_media_root):
    file_data = {"image": ("image.jpg", b"fake-image-bytes-content", "image/jpeg")}
    response = await client.put(f"/authors/{existing_author.id}/image", files=file_data, headers=admin_headers)
...
@pytest.mark.asyncio
async def test_update_author_image_incorrect_type(client, existing_author, admin_headers):
...
async def test_update_author_image_too_big(client, existing_author, admin_headers):
    file_data = {"image": ("image.jpg", b"0" * (settings.MAX_IMAGE_SIZE + 100), "image/jpeg")}
    response = await client.put(f"/authors/{existing_author.id}/image", files=file_data, headers=admin_headers)
...
@pytest.mark.asyncio
async def test_delete_author_image_by_admin_success(client, db_session, admin_headers, mock_media_root):
    author = Author(
        first_name="Лев",
        last_name="Толстой",
        bio="Биография",
...
@pytest.mark.asyncio
async def test_delete_author_image_missing_on_disk(client, db_session, mock_media_root, admin_headers):
    author = Author(
        first_name="Лев",
        last_name="Толстой",
        bio="Биография",
